...
#+begin_src emacs-lisp
  ...
  (defun fi/prova ()
    ...
    (setq width (- (window-body-width) 7))
    (setq top-border (concat "┌" (make-string width ?─) "┐\n"))
    (setq bottom-border (concat "└" (make-string width ?─) "┘\n"))
    (with-current-buffer "*scratch*"
      (insert top-border)
      (dolist (line (split-string content "\n"))
        (insert (concat "│ " line (make-string (- width (+(length line) 1)) ? ) "│\n")))
      )
